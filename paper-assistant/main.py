...
async def paper_detail(request: Request, paper_id: int):
    ...
    formulas = []
    if paper.get("figures") and paper["figures"] != "[]" and paper["figures"] != "0":
        try:
            formulas = json.loads(paper.get("figures", "[]"))
        ...
        if CLAUDE_API_KEY:
            try:
                ...
                update_paper(
                    ...
                    figures=json.dumps(analysis.get("formulas", []), ensure_ascii=False),
                    status="done",
                )
            ...
            update_paper(paper_id, sections=json.dumps([], ensure_ascii=False), status="done")
    except Exception as e:
    ...
    for pid in paper_ids:
        ...
        if sections_str and sections_str != "[]":
            try:
                ...
                paper_data["sections"] = cached
            except Exception:
                pass

        formulas_str = paper.get("figures", "[]")
        if formulas_str and formulas_str != "[]" and formulas_str != "0":
            try:
                parsed = json.loads(formulas_str)
                if isinstance(parsed, list):
                    paper_data["formulas"] = parsed
            except Exception:
                pass
